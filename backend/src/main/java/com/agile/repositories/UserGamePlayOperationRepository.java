...
public interface UserGamePlayOperationRepository extends JpaRepository<UserGamePlayOperation, Integer> {
	...
	@Query(value = "select new com.agile.repositories.CountGame(u.game.id, count(u.game.id)) from UserGamePlayOperation u group by u.game.id order by count(u.game.id) desc")
